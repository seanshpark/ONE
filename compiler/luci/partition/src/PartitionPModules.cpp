...
std::unique_ptr<loco::Graph> clone_graph(loco::Graph *graph_org, luci::CloneContext &clonectx)
{
  ...
  for (uint32_t n = 0; n < nodes->size(); ++n)
  {
    ...
    auto it = clonectx.find(node_org);
  ...
  for (uint32_t n = 0; n < nodes->size(); ++n)
  {
    ...
    // TODO enable recursive CircleIf
    assert(dynamic_cast<luci::CircleIf *>(node_org) == nullptr);
  }
                            ...
                            luci::CloneContext &clonectx)
{
  ...
  auto then_index = pm.module->add(std::move(then_clone));
  auto else_index = pm.module->add(std::move(else_clone));
  if_clone->then_branch(then_index);
  if_clone->else_branch(else_index);

  // if there was any new CircleIf inside then_graph or else_graph
